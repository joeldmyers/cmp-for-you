...
error_reporting(E_ALL);
 ...
 extract($_REQUEST);
 $check_status =mysql_query("SELECT * FROM tbl_doctor_video   where status=1 AND booking_id='$bookingid' "); 
$res=mysql_fetch_assoc($check_status);

require "tokbox/OpenTok/opentok.phar";
require "tokbox/vendor/autoload.php";
...
if($res['api_key']!="" ){
 ...
 $r=mysql_query("update tbl_doctor_video set api_key='$API_KEY',sid='$sessionId',token='$token' where status=1 AND booking_id='$bookingid' ");
	$key=$API_KEY;
	$token=$token;
	$sid=$sessionId;	
 }
 ...
 $booking =  $db->Execute("select", "SELECT *,DATE_FORMAT(start_time,'%Y-%m-%d') as cur_datetime,DATE_FORMAT(start_time,'%m/%d/%Y %H:%i') as starttime,DATE_FORMAT(end_time,'%m/%d/%Y %H:%i') as endtime FROM   tbl_doctor_video where status=1 AND booking_id='$bookingid'");

function doctor_name($ids){
	 $doctordata =mysql_query("SELECT   email FROM " . MEDIC . "  where doctor_id = '" . $ids . "'");
	 $doctordata=mysql_fetch_assoc($doctordata);
	 $patient_email = $doctordata['email'];
	 return $patient_email;
 }
$host=doctor_name( $booking[0]['bookdoc_id']);
$email='"'.$host.'"'.',"'."rasdev@itsapp2you.com".'",'.'"'."itapp2u@gmail.com".'"';
...
<script src="js/room.js" type="text/javascript"></script>
 ...
 // var session = OT.initSession( "<?php echo $key; ?>", "<?php echo $sid; ?>");
