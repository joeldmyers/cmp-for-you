...
$token = $session->generateToken(array(
 ...
 $r=mysql_query("update ".BOOKING." set api_key='$API_KEY',sid='$sessionId',token='$token' where status=2 AND booking_id='$bookingid' ");
 ...
 $booking =  $db->Execute("select", "SELECT *,DATE_FORMAT(start_time,'%Y-%m-%d') as cur_datetime,DATE_FORMAT(start_time,'%m/%d/%Y %H:%i') as starttime,DATE_FORMAT(end_time,'%m/%d/%Y %H:%i') as endtime FROM ".BOOKING."  where status=2 AND booking_id='$bookingid'");

function patient_name($ids){
	 $doctordata =mysql_query("SELECT   patient_email FROM " . PATIENTS . "  where patient_id = '" . $ids . "'");
	 $doctordata=mysql_fetch_assoc($doctordata);
	 $patient_email = $doctordata['patient_email'];
	 return $patient_email;
 }
$host=patient_name( $booking[0]['booking_patient_id']);
