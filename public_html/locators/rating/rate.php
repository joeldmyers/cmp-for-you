...
if(!$q)mysql_query("insert into ratings (id,date,user_id,doctor_id) values ($id_sent,curdate(),20,12)");
...
if(!$voted) {     //if the user hasn't yet voted, then vote normally...
	...
	if (($vote_sent >= 1 && $vote_sent <= $units)) { // keep votes within range, make sure IP matches 
		...
		$update = "UPDATE $rating_tableName SET total_votes='".$added."', total_value='".$sum."', used_ips='".$insertip."',user_id='20',doctor_id=12 WHERE id='$id_sent'";
		//echo $update;
