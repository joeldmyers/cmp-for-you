...
if($token_id){
	$invalid=1;
	$sql=mysql_query("SELECT * FROM " . TRANSACTIONS . "  WHERE transaction_type='person' AND patientid='" . $_SESSION['emp_id'] . "' AND doctorid='$doctor_id' AND transaction_status=1 AND transaction_number='".trim($token_id)."'");
...
if (isset($_POST['submit']) && $_POST['action'] == 'appiontment') {
	...
	if($pm){	
		...
		$endTime = date("Y-m-d H:i",strtotime("+30 minutes", strtotime($start_time)));
	}
	$db->Execute("insert", "insert into tbl_office_booking (booking_patient_id,bookdoc_id,patient_availability,date,start_time,end_time,transaction_number,type) values ('" . $_SESSION['emp_id'] . "' , '" . $docid . "','" . trim(mysql_real_escape_string($appointment_time)) . "','" . date('Y-m-d') ."','$start_time','$endTime','".$token_id."','Office') ");				}			/*********************************************************************************************/
 
    $headers = "MIME-Version: 1.0" . "\r\n";
    $headers .= "Content-type:text/html;charset=iso-8859-1" . "\r\n";
    $headers .= 'From: "' . $_SESSION['emp_email'] . '"' . "\r\n";
    $subject = "Office Appointment Confirmation";
...
<div class="col-md-5 col-xs-12">
    <h2 class="pad_btm20 pad_top10 pad_left10">Office Booking Confirmation</h2>
    <div class="searchbar">
