...
$errorMsg = '';
if(isset($_POST["forgotpass"]) && !empty($_POST["forgotpass"]))
{
        $emailid =(string)$_POST["email"];
        $isEmailexist = $db->Execute("select","SELECT `patient_email` FROM ".PATIENTS."  where `patient_email` = '".trim(mysql_real_escape_string($emailid))."'");
        if(isset($isEmailexist[0]['patient_email']) && !empty($isEmailexist[0]['patient_email']) && $isEmailexist[0]['patient_email'] == $_POST['email']){
        ...
        $mailmessage .= "<p><a href=".$remotelocation.'resetpatientpassword.php?token='.base64_encode($emailid)." target='_blank'>".$remotelocation."resetpatientpassword.php?token=".base64_encode($emailid)."</a></p><br/>";
        $mailmessage .= "<p><a href=".$remotelocation.'resetpatientpassword.php?token='.base64_encode($emailid)." target='_blank'><button style='align:center;' type='submit' id='button' name='forgotpass' class='btn-primary'>Change Password</button></a></p>";
        $mailmessage .= "<p><br /></p>";
...
<!--                            <div style="float:right; font-size:12px; position: relative;"><a href="#">Forgot password?</a></div>-->
                    ...
                    <input type="submit" name="forgotpass" class="btn-primary signup-btn" value="Submit" id="submit"  onclick="return checkforgotpass()"/>
                            </form>     
        </div> 
